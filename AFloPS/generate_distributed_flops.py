...
from utils import AFloPS_DIT as AFloPS_DIT
...
def main():
    ...
    parser.add_argument(
        "--c_max", type=float, default=1.0,
        help="lambda max"
    )
    parser.add_argument(
        "--mode", type=int, default=2,
        help="0:FloPS, 1:AFloPS"
    )
    args = parser.parse_args()


    save_path = os.path.join(args.save_dir, f"{args.sampler}_step{args.num_inference_steps}_scale{args.guidance_scale}_cmax{args.c_max}_mode{args.mode}")
    if rank == 0:

...
ORG_IMAGE_DIR="datasets/flickr30k/test"
PROMPT_DIR="datasets/flickr30k/test"
BASE_OUTPUT_DIR="generated_images_flickr30k"

MODEL_PATH=(
            'output/flickr30k_pretrained_sd35/'
            )

GAMMA=0.9
CURV_TH=1

OUTPUT_DIR="${BASE_OUTPUT_DIR}_lookahead_gamma${GAMMA}_curv${CURV_TH}_step${INFERENCE_STEPS}_g${GUIDANCE}"
...
python inference_sd35_lookahead.py \
    ...
    --scheduler=${SCHEDULER} \
    --seed 227 \
    --lookahead_backtrack_factor ${GAMMA} \
